...
fn wire_keccak256_encode_impl(port_: MessagePort, req: impl Wire2Api<KeccakReq> + UnwindSafe) {
    FLUTTER_RUST_BRIDGE_HANDLER.wrap(
        WrapInfo {
            debug_name: "keccak256_encode",
            port: Some(port_),
            mode: FfiCallMode::Normal,
        },
        move || {
            let api_req = req.wire2api();
            move |task_callback| Ok(keccak256_encode(api_req))
        },
    )
}
