...
use crate::wallet::coin_selection::OutputGroup;
    ...
    D: BatchDatabase,
{
    ...
    /// use bdk_lite::{KeychainKind, Wallet};
    /// use bdk_lite::database::MemoryDatabase;
    ...
    pub(crate) fn cal_fee(&self, params: TxParams) -> Result<u64, Error> {
        ...
        match (params.rbf, requirements.csv) {
        ...
        let (required_utxos, _) = self.preselect_utxos(
        ...
        let utxos_groups: Vec<OutputGroup> = required_utxos
            .into_iter()
            .map(|u| OutputGroup::new(u, fee_rate))
            .collect();

        let (utxo_fees, _) = utxos_groups
            .iter()
    ...
    /// # use bdk::*;
    /// # use bdk::database::*;
    /// use bdk_lite::{doctest_wallet, FeeRate, SignOptions};
    /// # let descriptor = "wpkh(tpubD6NzVbkrYhZ4Xferm7Pz4VnjdcDPFyjVu5K4iZXQ4pVN8Cks4pHVowTBXBKRhX64pkRyJZJN5xAKj4UDNnLPb5p2sSKXhewoYx5GbTdUFWq/*)";
    /// # let wallet = doctest_wallet!();
    /// # let to_address = Address::from_str("2N4eQYCbKUHCCTUjBJeHcJp9ok6J2GZsTDt").unwrap();
    /// let (mut psbt, _) = {
    ...
    /// let (mut psbt, _) =  {
    ///     let mut builder = wallet.build_fee_bump(tx.txid(), None)?;
    ///     builder
    ...
    pub fn build_fee_bump(
        ...
        if tx.output.len() > 1 {
            ...
            if keep_change.is_none() || keep_change.unwrap() == false {
    ...
    /// # use bdk::*;
    /// # use bdk::database::*;
    /// use bdk_lite::{doctest_wallet, SignOptions};
    /// # let descriptor = "wpkh(tpubD6NzVbkrYhZ4Xferm7Pz4VnjdcDPFyjVu5K4iZXQ4pVN8Cks4pHVowTBXBKRhX64pkRyJZJN5xAKj4UDNnLPb5p2sSKXhewoYx5GbTdUFWq/*)";
    /// # let wallet = doctest_wallet!();
    /// # let to_address = Address::from_str("2N4eQYCbKUHCCTUjBJeHcJp9ok6J2GZsTDt").unwrap();
    /// let (mut psbt, _) = {
    ...
    pub fn finalize_psbt(
        ...
        for (n, input) in tx.input.iter().enumerate() {
            ...
            let create_height = Some(u32::MAX);
            let last_sync_height = None;
    ...
    pub fn get_psbt_input(
        ...
        txs.iter()
            .find(|d| d.txid() == prev_output.txid)
            .map(|prev_tx| {
...
pub(crate) mod test {
    ...
    fn test_bump_fee_irreplaceable_tx() {
        ...
        wallet
            .build_fee_bump(txid, Some(false))
            .unwrap()
            .finish()
            .unwrap();
    }
    ...
    fn test_bump_fee_low_fee_rate() {
        ...
        let mut builder = wallet.build_fee_bump(txid, Some(false)).unwrap();
