...
//! use bdk_lite::{doctest_wallet, FeeRate};
...
use bitcoin::{LockTime, OutPoint, Script, Sequence, Transaction, TxOut};
...
/// use bdk_lite::wallet::tx_builder::TxOrdering;
/// # let wallet = doctest_wallet!();
    ...
    TxBuilder<'a, D, Cs, Ctx>
{
    ...
    /// # use bitcoin::*;
    /// # use bdk::*;
    /// use bdk_lite::{doctest_wallet, KeychainKind};
    /// # let to_address = Address::from_str("2N4eQYCbKUHCCTUjBJeHcJp9ok6J2GZsTDt").unwrap();
    /// # let wallet = doctest_wallet!();
    ...
    /// add tsx
    pub fn add_txs(&mut self, txs: &[Transaction]) -> Result<&mut Self, Error> {
        for tx in txs {
            self.params.txs.push(tx.clone());
        }
        Ok(self)
    }

    /// document
    pub fn add_utxo_foreign(&mut self, foreign_utxos: &[ForeignUtxo]) -> Result<&mut Self, Error> {
        let utxos = foreign_utxos
            .iter()
            .map(|foreign_utxo| LocalUtxo {
                outpoint: foreign_utxo.outpoint,
                txout: TxOut {
                    value: foreign_utxo.txout.value,
                    script_pubkey: Script::from_str(&foreign_utxo.txout.script_pubkey).unwrap(),
                },
    ...
    pub fn cal_fee(self) -> Result<u64, Error> {
        self.wallet.cal_fee(self.params)
    }
